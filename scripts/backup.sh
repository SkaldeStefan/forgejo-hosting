...
main() {
  ...
  backup_basename="forgejo-backup-${timestamp}.tar.gz"
  ...
  files_archive_path="$tmp_dir/forgejo-files.tar.gz"
  : > "$tmp_dir/include-list.txt"
  for file in docker-compose.yml .env forgejo-config forgejo-data; do
...
EOF
  ...
  tar -C "$tmp_dir" -czf "$backup_path" postgres.dump forgejo-files.tar.gz metadata.env
  chmod 600 "$backup_path"

  find "$BACKUP_DIR" -maxdepth 1 -type f -name 'forgejo-backup-*.tar.gz' -mtime "+$RETENTION_DAYS" -delete
