...
Examples:
  scripts/restore.sh backups/forgejo-backup-20260305T150000Z.tar.gz
  scripts/restore.sh --force forgejo-backup-20260305T150000Z.tar.gz
EOF
}
...
main() {
  ...
  [ -f "$tmp_dir/forgejo-files.tar.gz" ] || { echo "ERROR: forgejo-files.tar.gz missing in archive." >&2; exit 1; }
  ...
  compose_cmd stop forgejo >/dev/null 2>&1 || true
  compose_cmd up -d postgres >/dev/null
  wait_for_postgres

  echo "Restoring project files..."
  tar -xzf "$tmp_dir/forgejo-files.tar.gz" -C "$PROJECT_DIR"
